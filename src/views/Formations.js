...
const FormItem = ({ id, photoUrl, name, description, deleteFormation }) => {
	return (
		<Col
			className="d-flex align-items-center bg-white m-2 p-2"
		...
		>
			...
			<Col md="2" sm="12" lg="2">
				<Button
					onClick={() => deleteFormation(id)}
					className="ml-1 bg-danger border-danger"
					color="red"
				>
	...
	return (
		<Container fluid className="main-content-container px-4 pb-4">
			...
			<Col>
				{data &&
					data.getFormations &&
					data.getFormations.map((item) => (
						<FormItem
							deleteFormation={deleteFormationx}
							key={item.id}
							{...item}
						/>
					))}
				{data && data.getFormations && data.getFormations.length == 0 && (
					<div 
